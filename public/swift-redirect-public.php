...
if (!class_exists('SwiftRedirectPublic')) {

    class SwiftRedirectPublic{
        ...
        public function run_SwiftRedirect(){
            ...
            $redirects_list = json_decode(json_encode($this->redirects), true);

            foreach ($redirects_list as $rule) {

                $rule_object = new SwiftRedirectInstance($rule);
                ...
                if($rule_object->is_enabled){
                    ...
                    if($rule_object->is_params){
                        $target .= '?' . parse_url($path, PHP_URL_QUERY);
                    }
